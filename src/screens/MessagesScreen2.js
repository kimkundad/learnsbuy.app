...
class ChatScreen2 extends Component {
    ...
    return (
      <ChatComponent dataParams={this?.props?.route?.params?.userchat}/>
    );
  }

}

function ChatComponent(dataParams) {
    ...
    const getDataUser = async () => {
        console.log('prop', dataParams)
        try {
              const response = await axios.get(`https://www.learnsbuy.com/api/get_userby_id/${dataParams.dataParams}`)
      ...
      console.log('user', toUser?.fcmToken)
    ...
    useEffect(() => {
      ...
      const handleIncomingMessage = (message) => {
        ...
        if(message.chat_user_id == user?.profile?.id || message.agent_id == user?.profile?.id){
            ...
            console.log('------------->recive sockerData', sockerData)
        ...
        const fetchChatMessages = async () => {
            try {
              ...
              //  const response = await axios.post('http://10.0.2.2:3002/',{token})
                const response = await axios.get(`https://api.learnsabuy.com/?page=1&userID=${dataParams.dataParams}`)
            ...
            <TouchableOpacity style={styles.sendButton}
                onPress={ async () => {
                    ...
                    if (isAttachImage) {
                        ...
                        formData.append('agent_id', dataParams.dataParams);
                        ...
                        if (inputText.trim().length > 0) {
                            ...
                            try {
                                ...
                                const responsex = await axios.post('https://api.learnsabuy.com/index_2', {
                                  ...
                                  agent_id: dataParams.dataParams,
